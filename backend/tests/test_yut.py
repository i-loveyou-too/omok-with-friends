...
def test_card_movement_captures_and_grants_extra_turn(monkeypatch):
    ...
    # O1->O2 (special) captures "theirs" and chains one more step to O3, which
    # isn't itself a lucky-mode special tile, so the chain stops there.
...
def test_yut_api_join_and_reconnect_state_sync():
    response = client.post("/omokwithfriend/api/yut/rooms?mode=lucky")
