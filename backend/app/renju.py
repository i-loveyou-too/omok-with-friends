...
class RenjuRuleEngine:
    """Renju-style rules for a 15x15 board.

    Black wins with exactly five and cannot play overlines, double-fours or
    double-threes. White has no forbidden moves and wins with five or more.
    """
    ...
    def analyze_black_move(self, board: Board, row: int, col: int) -> MoveAnalysis:
        ...
        board[row][col] = BLACK
        try:
            if self._has_overline(board, row, col):
                return MoveAnalysis(ForbiddenReason.OVERLINE)

            winning = self.winning_line(board, row, col, BLACK)
            # In Renju, a legal exact-five wins even when it also resembles a
            # double-three or double-four. Overline remains forbidden.
            if winning:
                return MoveAnalysis(None, tuple(winning))

            if self._four_threats(board, row, col) >= 2:
                return MoveAnalysis(ForbiddenReason.DOUBLE_FOUR)
            if self._open_three_directions(board, row, col) >= 2:
                return MoveAnalysis(ForbiddenReason.DOUBLE_THREE)
            return MoveAnalysis(None)
        finally:
            board[row][col] = EMPTY

    def forbidden_points(self, board: Board) -> List[Tuple[int, int, str]]:
        points: List[Tuple[int, int, str]] = []
        for row in range(BOARD_SIZE):
            for col in range(BOARD_SIZE):
                if board[row][col] is EMPTY:
                    reason = self.analyze_black_move(board, row, col).forbidden
                    if reason:
    ...
    def winning_line(
        ...
        for dr, dc in DIRECTIONS:
            ...
            if (color == BLACK and len(run) == 5) or (color == WHITE and len(run) >= 5):
                return run
        return []
    ...
    def _has_overline(self, board: Board, row: int, col: int) -> bool:
        return any(
            len(self._contiguous_run(board, row, col, BLACK, dr, dc)) >= 6
            for dr, dc in DIRECTIONS
        )
    ...
    def _exact_five_sets(
        self, board: Board, origin: Coord, direction: Coord
    ) -> Set[frozenset[Coord]]:
        ...
        for start in range(0, len(coords) - 4):
            ...
            if origin not in window or any(board[r][c] != BLACK for r, c in window):
                continue
            left = coords[start - 1] if start > 0 else None
            right = coords[start + 5] if start + 5 < len(coords) else None
            if left and board[left[0]][left[1]] == BLACK:
                continue
            if right and board[right[0]][right[1]] == BLACK:
                continue
    ...
    def _winning_extensions(
        ...
        for r, c in self._line_coords(row, col, dr, dc):
            ...
            board[r][c] = BLACK
            try:
                for five in self._exact_five_sets(board, origin, direction):
                    if (r, c) in five:
    ...
    def _four_threats(self, board: Board, row: int, col: int) -> int:
        threats: Set[Tuple[Coord, frozenset[Coord]]] = set()
        for direction in DIRECTIONS:
            for _, stones in self._winning_extensions(board, (row, col), direction):
                if (row, col) in stones:
                    threats.add((direction, stones))
        return len(threats)

    def _open_three_directions(self, board: Board, row: int, col: int) -> int:
        count = 0
        origin = (row, col)
        for direction in DIRECTIONS:
            dr, dc = direction
            ...
            for r, c in self._line_coords(row, col, dr, dc, radius=4):
                ...
                try:
                    ...
                    wins = self._winning_extensions(board, origin, direction)
                    endpoints = {point for point, stones in wins if (r, c) in stones}
                    if len(endpoints) >= 2 and not self._has_overline(board, r, c):
                        found = True
                        break
                finally:
                    board[r][c] = EMPTY
