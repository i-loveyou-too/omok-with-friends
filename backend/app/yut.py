...
ALL_TILES = {*[f"O{i}" for i in range(1, 21)], "A1", "A2", "A3", "A4", "B1", "B2", "B3", "B4", "C"}
# 29 tiles total (excluding S/F). Tuned so the density gap is clearly felt:
# classic ~6 special tiles (a light sprinkle, board stays mostly a normal yut
# board), lucky ~15 (about half the board), never so dense it stops feeling
# like yut. See the "특수칸 밀도" tuning pass — keep future edits inside
# these ranges (classic 5-7, lucky 14-17) rather than the board's full 29.

# Classic mode stays close to a traditional yut board: only a light sprinkle of
# lucky-card tiles so the game still feels "정통 윷놀이" most of the time.
CLASSIC_LUCKY_LOCATIONS = {"O9", "O19", "A3"}
CLASSIC_JACKPOT_LOCATIONS = {"C", "B2"}
CLASSIC_DANGER_LOCATIONS = {"O16"}

# Lucky mode is noticeably busier than classic (roughly half the board is
# special) but still leaves plenty of plain tiles so it stays recognizably yut.
LUCKY_JACKPOT_LOCATIONS = {"C", "O5", "O15", "A2", "B2"}
LUCKY_DANGER_LOCATIONS = {"O10", "O18", "A4", "B4"}
LUCKY_LUCKY_LOCATIONS = {"O2", "O4", "O7", "O9", "A1", "A3"}

MODE_TILES = {
    "classic": (CLASSIC_LUCKY_LOCATIONS, CLASSIC_JACKPOT_LOCATIONS, CLASSIC_DANGER_LOCATIONS),
    "lucky": (LUCKY_LUCKY_LOCATIONS, LUCKY_JACKPOT_LOCATIONS, LUCKY_DANGER_LOCATIONS),
}

# Kept for backwards compatibility with anything importing the old fixed sets;
# these mirror the (denser) lucky-mode tile layout.
LUCKY_LOCATIONS = LUCKY_LUCKY_LOCATIONS
JACKPOT_LOCATIONS = LUCKY_JACKPOT_LOCATIONS
DANGER_LOCATIONS = LUCKY_DANGER_LOCATIONS
MAX_CARD_CHAIN = 4
...
class YutRoom:
    def __init__(self, room_code: str, mode: str = "lucky") -> None:
        ...
        self.lucky_locations, self.jackpot_locations, self.danger_locations = MODE_TILES[self.mode]
    ...
    def _weighted_card(self, location: str) -> LuckyCard:
        if location in self.jackpot_locations:
            pool = tuple(card for card in CARD_DEFINITIONS if card.tier == "✨")
        elif location in self.danger_locations:
            pool = tuple(card for card in CARD_DEFINITIONS if card.tier == "💀")
        else:
            pool = tuple(card for card in CARD_DEFINITIONS if card.tier == "🍀")
    ...
    def _resolve_lucky_chain(self, piece: Piece, depth: int) -> tuple[bool, List[dict]]:
        loc = piece.location
        if loc not in self.lucky_locations | self.jackpot_locations | self.danger_locations:
            return False, []
    ...
    def snapshot(self) -> dict:
        ...
        return {
            ...
            "lucky": {
                "normal": sorted(self.lucky_locations),
                "jackpot": sorted(self.jackpot_locations),
                "danger": sorted(self.danger_locations),
            },
