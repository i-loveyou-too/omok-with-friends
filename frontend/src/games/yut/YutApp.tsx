...
const API = (import.meta.env.VITE_API_BASE as string | undefined)?.replace(/\/$/, '') ?? `${APP_BASE}/api`
...
function YutLobby({ onEnter }: { onEnter: (code: string) => void }) {
  ...
  return <main className="yut-page yut-lobby">
    <section className="yut-card yut-hero">
      ...
      <a className="yut-back-link" href={`${APP_BASE}/`}>← 먼작귀게임방으로</a>
    </section>
  </main>
}
...
function YutGame({ roomCode, profile, onSession, onLeave }: { roomCode: string; profile: YutProfile; onSession: (s: YutSession) => void; onLeave: () => void }) {
  ...
  if (!state) return <main className="yut-page yut-loading"><img className="yut-loading-stick" src={yutAssets.results.yut} alt="윷"/><h1>윷판 펴는 중…</h1>{error && <div className="yut-toast">{error}</div>}</main>
  ...
  const copy = async () => { await navigator.clipboard.writeText(`${location.origin}${APP_BASE}/yut/room/${roomCode}`); setCopied(true); setTimeout(() => setCopied(false), 1600) }
  return <main className="yut-page yut-game">
    ...
    <section className="yut-main-grid"><YutBoard state={state} selfId={selfId} canMove={canMove} onMove={(id) => send({ type: 'move', pieceId: id })} overlay={
        <div className="yut-board-overlay">
          {myTurn && !state.pendingRoll && !isRolling && <div className="yut-turn-banner" role="status"><b>내 차례예요!</b><span>윷을 던지세요</span></div>}
          {isRolling && <div className="yut-throw-stage" aria-label="윷을 던지는 중"><span className="sr-only">윷을 던지는 중</span>{[0, 1, 2, 3].map((stick) => <img key={stick} style={{ '--stick': stick } as CSSProperties} src={resultAsset ?? yutAssets.results.do} alt=""/>)}</div>}
          {state.pendingRoll && !isRolling && <div className="yut-roll-readout" role="status">{resultAsset && <img src={resultAsset} alt=""/>}<b>{ROLL_LABEL[state.pendingRoll.name]}</b><small>{state.pendingRoll.steps}칸 · {myTurn ? '움직일 말을 골라줘!' : '상대가 고르는 중…'}</small></div>}
          {myTurn && state.status === 'playing' && <button className="yut-primary yut-board-roll-btn" disabled={Boolean(state.pendingRoll) || isRolling} onClick={roll}>{isRolling ? <span>윷이 날아간다!</span> : state.pendingRoll ? <span>말을 골라줘!</span> : <img src={yutAssets.ui.rollButton} alt="윷 던지기!"/>}</button>}
        </div>
      }/>
    ...
    {error && <div className="yut-toast">{error}</div>}
  </main>
}
