...
export function YutBoard({ state, selfId, canMove, onMove, overlay }: Props) {
  ...
  return (
    <div className="yut-board-wrap">
      <div className="yut-board" aria-label="윷놀이 말판">
        ...
        <div className="yut-start-flag" style={{ left: `${BOARD_POSITIONS.S[0]}%`, top: `${BOARD_POSITIONS.S[1]}%` }} aria-label="출발·도착 지점">
          <b aria-hidden="true">🏁</b><span>S·F</span>
        </div>
        {groups.map((group) => {
          const first = group[0]
        ...
        <div className="yut-home yut-home--me"><img className="yut-start-art" src={yutAssets.ui.start} alt=""/><b>출발·도착</b>{selfHome.map((p) => <button key={p.id} aria-label={`내 말 ${p.id + 1}`} disabled={!canMove} onClick={() => onMove(p.id)}>{selfPlayer && <CharacterAvatar character={selfPlayer.character}/>}</button>)}</div>
