...
import { BalloonLobby } from './components/BalloonLobby'
...
type Route =
  ...
  | { kind: 'balloon-room'; roomCode: string | null; invalid: boolean }
...
function routeFromPath(): Route {
  ...
  if (normalized === `${APP_BASE}/balloon`) return { kind: 'balloon' }
  ...
  const balloonMatch = location.pathname.match(new RegExp(`${escaped}/balloon/room/([^/?#]+)`, 'i'))
  ...
  const match = balloonMatch ?? findMatch ?? secretMatch ?? omokMatch
  if (!match) return { kind: 'hub' }
  const roomCode = decodeURIComponent(match[1]).trim().toUpperCase()
  const valid = /^[A-Z0-9]{5}$/.test(roomCode)
  if (balloonMatch) return { kind: 'balloon-room', roomCode: valid ? roomCode : null, invalid: !valid }
...
type RoomRouteKind = 'omok-room' | 'secret-card-room' | 'find-match-room' | 'balloon-room'

function sessionKey(kind: RoomRouteKind, roomCode: string) {
  const game = kind === 'omok-room' ? 'omok' : kind === 'secret-card-room' ? 'secret-card' : kind === 'find-match-room' ? 'find-match' : 'balloon'
...
export default function App() {
  ...
  const roomRoute = route.kind === 'omok-room' || route.kind === 'secret-card-room' || route.kind === 'find-match-room' || route.kind === 'balloon-room' ? route : null
  ...
  const applyRoute = useCallback((next: Route) => {
    setRoute(next)
    const nextRoom = next.kind === 'omok-room' || next.kind === 'secret-card-room' || next.kind === 'find-match-room' || next.kind === 'balloon-room' ? next : null
  ...
  useEffect(() => {
    ...
    const endpoint = roomRoute.kind === 'secret-card-room'
      ? 'secret-card/rooms'
      : roomRoute.kind === 'find-match-room' ? 'find-match/rooms' : roomRoute.kind === 'balloon-room' ? 'balloon/rooms' : 'rooms'
  ...
  const goBalloon = () => navigate(`${APP_BASE}/balloon`, { kind: 'balloon' })
  ...
  const enterBalloon = (roomCode: string) => navigate(`${APP_BASE}/balloon/room/${roomCode}`, { kind: 'balloon-room', roomCode, invalid: false })
  ...
  const roomHome = roomRoute?.kind === 'secret-card-room' ? goSecretCard : roomRoute?.kind === 'find-match-room' ? goFindMatch : roomRoute?.kind === 'balloon-room' ? goBalloon : goOmok

  if (roomMissing) return <EmptyRoom onHome={roomHome} />
  if (route.kind === 'hub') return <MinigameHub onOmok={goOmok} onSecretCard={goSecretCard} onYut={goYut} onFindMatch={goFindMatch} onCurling={goCurling} onBalloon={goBalloon} />
  ...
  if (route.kind === 'balloon') return <BalloonLobby onEnter={enterBalloon} onBack={goHub} />
  ...
  if (roomRoute.kind === 'balloon-room') {
    return <BalloonRoom roomCode={roomRoute.roomCode} profile={profile} onSession={(session) => saveSession('balloon-room', session)} onLeave={goBalloon} onRoomMissing={() => setRoomMissing(true)} />
  }
